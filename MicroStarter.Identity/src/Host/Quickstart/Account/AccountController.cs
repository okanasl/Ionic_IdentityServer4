    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginInputModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var result = await _signInManager.PasswordSignInAsync(userName, model.Password, model.RememberLogin, lockoutOnFailure: false);

                
                if (result.Succeeded)
                {
                    _logger.LogInformation(1, $"User with username = {userName} logged in. ");
                ...
                {
                    _logger.LogWarning(2, $"User with username = {userName} account locked out.");
                    return View("Lockout");
                }
                else
                {
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    {
                        var role =await _roleManager.FindByNameAsync("UI_Admin");
                        if( role== null)
                        {
